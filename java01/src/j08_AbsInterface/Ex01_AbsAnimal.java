...
class Bear {	// 독립적으로 만들어진 클래스 => animal의 계보에 들어갈 수 없음
...
public class Ex01_AbsAnimal {
	...
	public static void main(String[] args) {
		// 1) Animal 생성하기
		// => 직접 인스턴스를 생성하지 못함. ( new 사용불가 ) >> 구현체에 대한 객체를 생성하여, 인터페이스 메서드에 접근
		...
		System.out.println("★★★★Animal Type 으로 객체 생성");
		Animal c1 = new Cat();
		c1.breath();
		c1.sound();
		
		// 2.2) Cat Type 으로 생성
		System.out.println("★★★★Cat Type 으로 객체 생성");
		Cat c2 = new Cat();
		c2.breath();
		c2.sound();
		c2.eyeColor();
		...
		System.out.println("★★★★Dog Type 으로 객체 생성");
		Dog d2 = new Dog();
		d2.breath();
		d2.sound();
		d2.speed();
