...
// 페이지 객체에 담겨있는 DTO와 Entity(EN) Type을 가져다쓴다..?
@Data
public class PageResultDTO<DTO, EN> {
	// => DTO, Entity, Type을 지정

	// DTO List
	// 클래스 내부에서 정의된 DTO 값을 받아오는 것
	...
	private int start, end; // 시작 페이지, 종료 페이지
	private boolean prev, next; // 앞으로가기, 뒤로가기
	...
	private void makePageList(Pageable pageable) {
		...
		prev = start > 1;	// true or false가 prev에 들어감
		...
		// 즉, int 스트림의 start 부터 end까지를 일반 스트림으로 변환하는 것이다.
	}
