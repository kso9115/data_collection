...
//------------------------------------------------------------------------------------
	...
	public String join(Model model, MemberDTO dto, HttpServletRequest request) throws IOException {
		...
		if (!realPath.contains("-tomcat-")) // 개발중
			...
			realPath ="E:\\ksoo\\IDESet\\apache-tomcat-9.0.85\\webapps\\demoM\\resources\\uploadImages\\"; // 배포 후
		...
		if (!file.isFile()) { // 존재하지않는 경우(파일존재의 여부를 확인) -> 디렉토리의 경로를 확인해주는 것
			String basicImagePath = "E:\\ksoo\\gitANDeclipse\\demoM\\src\\main\\webapp\\resources\\images\\basicman4.png";
	...
	public String update(HttpSession session, Model model, MemberDTO dto, HttpServletRequest request)
		...
		if (uploadfilef != null && !uploadfilef.isEmpty()) {
			...
			if (!realPath.contains("-tomcat-")) // 개발중
				realPath += "resources\\uploadImages\\"; // 현재 파일의
																												// 경로
			else
				realPath = "E:\\ksoo\\IDESet\\apache-tomcat-9.0.85\\webapps\\demoM\\resources\\uploadImages\\"; // 배포
																													// 후
																													// 서버에
																													// 저장되는
																													// 경로
